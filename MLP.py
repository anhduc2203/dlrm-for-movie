...
class MLP(nn.Module):
    ...
    def __init__(self, n_users, n_items, layers=[16, 8], dropout=False, timestamp=None):
        """
    ...
    def forward(self, feed_dict):
        ...
        times = np.ones(shape=(len(items)))
        for t in range(len(times.shape)):
            times[t] = self.timestamp[items[t]]
...
def main():
    ...
    train, testRatings, testNegatives, timestamp = full_dataset.trainMatrix, full_dataset.testRatings, \
                                                   full_dataset.testNegatives, full_dataset.timestamp
    ...
    # Build model
    model = MLP(num_users, num_items, layers=layers, dropout=dropout, timestamp=timestamp)
    ...
    for epoch in range(epochs):
        ...
        if epoch % verbose == 0:
            ...
            MSE_loss_list.append(epoch_loss)
            # if hr > best_hr:
    ...
    print("loss for epochs: ", MSE_loss_list)
