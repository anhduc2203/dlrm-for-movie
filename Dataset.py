...
class MovieLensDataset(Dataset):
    ...
    def __init__(self, file_name, num_negatives_train=5, num_negatives_test=100):
        ...
        self.trainMatrix, self.timestamp = self.load_rating_file_as_matrix(file_name + ".train.rating")
    ...
    def load_rating_file_as_matrix(self, filename):
        ...
        timestamp = dict()
        max_timestamp, min_timestamp = 0, 893286638
        with open(filename, "r") as f:
            line = f.readline()
            while line != None and line != "":
                arr = line.split("\t")
                u, i = int(arr[0]), int(arr[1])
                timestamp[i] = int(arr[3].strip("\n"))
                max_timestamp = max(timestamp[i], max_timestamp)
                min_timestamp = min(timestamp[i], min_timestamp)
                num_users = max(num_users, u)
                num_items = max(num_items, i)
                line = f.readline()

        for i, t in timestamp.items():
            timestamp[i] = (t - min_timestamp) / (max_timestamp - min_timestamp)
